...
export class ImageField extends Field {
    ...
    createUi(container) {
        ...
        var fieldOuter = $(".form-field-outer");
        ...
        this.el_ = $("<div>")
            ...
            .addClass("form-field-buttonset border rounded")
        ...
        if (!this.params_.imageOnly) {
            let clipartParamsEl = $("<div>")
                .addClass(
                    "form-image-type-params form-image-type-params-clipart is-hidden border rounded"
                )
                .appendTo(fieldContainer);
    ...
    setupDropTarget_(el) {
        ...
        $el.addClass("form-field-drop-target")
            .on("dragenter", (ev) => {
                ev.preventDefault();
        ...
        this.renderSource_()
            .then(({ ctx, size }) => {
                this.computeTrimRect_(ctx, size).then((trimRect) => {
                    ...
                    let padPx = Math.round(
                        pad * Math.min(trimRect.w, trimRect.h)
                    );
                    ...
                    if (this.imagePreview_) {
                        ...
                        let previewCtx = this.imagePreview_
                            .get(0)
                            .getContext("2d");
            ...
            switch (this.valueType_) {
                case "image":
                    if (this.imageParams_.uri) {
                        Util.loadImageFromUri(this.imageParams_.uri).then(
                            (img) => {
                                ...
                                if (
                                    this.imageParams_.isSvg &&
                                    this.params_.maxFinalSize
                                ) {
                                    if (
                                        size.w / size.h >
                                        this.params_.maxFinalSize.w /
                                            this.params_.maxFinalSize.h
                                    ) {
                                        size.w = this.params_.maxFinalSize.w;
                                        size.h =
                                            (size.w * origSize.h) / origSize.w;
                                    } else {
                                        size.h = this.params_.maxFinalSize.h;
                                        size.w =
                                            (size.h * origSize.w) / origSize.h;
                                    }
                                }
        ...
        let vals = {
            ...
            text:
                this.valueType_ == "text"
                    ? this.textForm_.getValuesSerialized()
                    : null,
        };
...
ImageField.loadImageFromFileList = function (fileList) {
    return new Promise((resolve, reject) => {
        ...
        let file = Array.from(fileList).find((file) =>
            ImageField.isValidFile_(file)
        );

        if (!file) {
            alert(
                "Please choose a valid image file (PNG, JPG, GIF, SVG, etc.)"
            );
            resolve(null);
            return;
        }
